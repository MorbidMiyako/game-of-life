            ...
            <div key={i} style={inlineStyles.gameRow}>
              {row.map((tile, j) => {
                return (
                  ...
                  tile ? (
                    <div key={j} style={inlineStyles.trueTile} onClick={() => {
                      // update both the array of the gameboard and the tiles that are true
                      setGenerationCount(0)
                      setGameBoard([...gameBoard], gameBoard[i][j] = false)
                      delete trueTiles[`${i}${j}`]
                    }} />
                  ) : (
                      <div key={j} style={inlineStyles.falseTile} onClick={() => {
                        setGenerationCount(0)
                        setGameBoard([...gameBoard], gameBoard[i][j] = true)
                        trueTiles[`${i}${j}`] = [i, j]
                      }} />
                    )
                )
