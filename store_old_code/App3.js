...
function App() {
  ...
  const [generationCount, setGenrationCount] = useState(0)
  const [play, setPlay] = useState(false)

  const NextStep = () => {
    let returnedArray = RedrawGameBoard(gameBoard, trueTiles)
    setGameBoard(returnedArray[0])
    trueTiles = returnedArray[1]
    setGenrationCount(generationCount + 1)
  }

  const ChangeClass = (i, j) => {
    let element = document.getElementById(`F${i}${j}`)
    console.log(element)
  ...
  return (
    <div className="App">
      ...
      <div style={inlineStyles.gameBoard}>
        ...
        <p>Current generation:{generationCount}</p>
        <button onClick={NextStep}>Next Step</button>
        <button onClick={e => setPlay(true)}>Play</button>
        <button onClick={e => setPlay(false)}>Stop</button>
        <button onClick={e => {
          setGameBoard(CreateGameBoard(gameBoard.length, gameBoard[0].length))
        }}>Clear</button>
      </div>
    </div>
  );
